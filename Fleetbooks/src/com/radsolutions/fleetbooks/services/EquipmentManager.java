...
public class EquipmentManager {
	...
	private static final String EDIT_EQUIPMENT = "UPDATE equipment SET companyid = ?, brand=?, model=?, year=?, type=?, vin=?, value=?, usagehour=?, monthlycost=?, weeklycost=?, dailycost=?, note=?, isactive=?, isavailable=?, isbilling=?, projectid=?, photo=? WHERE id = ?";
	private static final String DELETE_EQUIPMENT = "UDATE equipent SET isActive = FALSE WHERE id =?";
	...
	public boolean editEquipment(Equipment eq){
		Connection conn = null;
		int status;
		try{
			conn = DataSource.getInstance().getJDBCConnection();
			
			PreparedStatement stmt = conn.prepareStatement(EDIT_EQUIPMENT);
			//(companyid, brand, model, year, type, vin, value, usagehour, monthlycost, 
			//weeklycost, dailycost, note, isactive, isavailable, isbilling, projectid, 
			//photo)
			stmt.setString(1, eq.getCompanyId());
			...
			stmt.setDouble(11, eq.getDailyCost());
			stmt.setString(12, eq.getNote());
			stmt.setBoolean(13, eq.isActive());
			stmt.setBoolean(14, eq.isAvailable());
			stmt.setBoolean(15, eq.isBilling());
			stmt.setInt(16, eq.getLocationEquipmentID());
			stmt.setString(17, eq.getPictureLink());
			stmt.setInt(18, eq.getId());
	...
	public boolean deleteEquipment(int id) {
		...
		try{
			...
			PreparedStatement stmt = conn.prepareStatement(DELETE_EQUIPMENT);
			stmt.setInt(1, id);
