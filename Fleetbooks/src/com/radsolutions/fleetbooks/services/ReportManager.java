...
public class ReportManager {
	...
	private static final String GET_EQUIPMENT_USAGE_REPORT = "SELECT U.*, E.companyid FROM uses AS U, equipment AS E WHERE U.equipmentid = E.id AND U.projectid = ? AND U.date BETWEEN ? AND ? ORDER BY U.equipmentid, U.date ASC ";
	...
	private static final String GET_FUEL_CONSUMPTION_REPORT = "SELECT records.*,MAX(F.date), F.price FROM records, fuelpurchase AS F WHERE F.projectid = records.projectid  AND F.date < records.date GROUP BY F.price, records.projectid, records.equipmentid, records.date, records.gallons;";
	
	private static final String SUBMIT_FUEL_PURCHASE_REPORT = "INSERT INTO fuelpurchase (date, gallons, price, projectid) VALUES (?, ?, ?, ?)";
	private static final String SUBMIT_EQUIPMENT_USAGE_REPORT = "INSERT INTO uses (projectid, equipmentId, date, usage) VALUES (?, ?, ?, ?)";
	private static final String SUBMIT_FUEL_CONSUMPTION_REPORT = "INSERT INTO refuels (projectid, equipmentId, date, gallons) VALUES (?, ?, ?, ?)";
	
	private static final String DROP_FUEL_VIEW = "DROP VIEW records;";
	private static final String CREATE_FUEL_VIEW ="CREATE VIEW records AS SELECT * FROM refuels WHERE equipmentid = ? AND date BETWEEN ? AND ?;" ;
	...
	public ResultSet getFuelConsumptionReport(String startDate, String endDate, int equipmentId){
		...
		Connection conn = null;
		try {
			conn = DataSource.getInstance().getJDBCConnectionAcceptMultipleQueries();
			PreparedStatement stmt = conn.prepareStatement(DROP_FUEL_VIEW);
			stmt.execute();
			
			stmt = conn.prepareStatement("CREATE VIEW records AS SELECT * FROM refuels WHERE equipmentid = "+equipmentId+" AND date BETWEEN '"+startDate+"' AND '"+endDate+"';");
			/*stmt.setInt(1, equipmentId);
			stmt.setDate(2,  Date.valueOf(startDate));
			stmt.setDate(3, Date.valueOf(endDate));*/
			stmt.execute();
			
			stmt = conn.prepareStatement(GET_FUEL_CONSUMPTION_REPORT);
			result = stmt.executeQuery();
		}
		catch(Exception e){
			System.out.println("Unable to read data from data source: "+ e);	
	...
	/**
	 * Inserts to the database records of equipment usage for a specific equipment in a project
	 * @param date when the usage took place
	 * @param projectId id of project
	 * @param equipmentId id of equipment
	 * @param usage hours of use for the equipment
	 */
	public void submitEquipmentUsageReport(String date, int projectId, int equipmentId, int usage){
		Connection conn = null;
		try {
			conn = DataSource.getInstance().getJDBCConnection();
			PreparedStatement stmt = conn.prepareStatement(SUBMIT_EQUIPMENT_USAGE_REPORT);
			stmt.setInt(1,  projectId);
			...
			System.out.println("Unable to insert data to data source: "+ e);
		}
		finally {
			this.closeConnection(conn);
		}
	...
	/**
	 * Inserts to the database records of fuel consumption
	 * @param date date when the fuel was consumed
	 * @param projectId projectId of project where equipment is assigned to
	 * @param equipmentId equipmentId of equipment using fuel
	 * @param gallons quantity of fuel used in gallons
	 */
	public void submitFuelConsumptionReport(String date, int projectId, int equipmentId, int gallons){
		Connection conn = null;
		try {
			conn = DataSource.getInstance().getJDBCConnection();
			PreparedStatement stmt = conn.prepareStatement(SUBMIT_FUEL_CONSUMPTION_REPORT);
			stmt.setInt(1,  projectId);
			stmt.setInt(2, equipmentId);
			stmt.setDate(3, Date.valueOf(date));
			stmt.setInt(4,  gallons);
			stmt.executeUpdate();
		}
		catch(Exception e){
			System.out.println("Unable to insert data to data source: "+ e);
	...
	/**
	 * Inserts to the database records of fuel purchase
	 * @param date when the purchase was made
	 * @param gallons quantity of fuel purchased in gallons
	 * @param price fuel purchase price at date given
	 */
	public void submitFuelPurchaseReport(String date, int projectId, int gallons, float price){
		Connection conn = null;
		try {
			conn = DataSource.getInstance().getJDBCConnection();
			PreparedStatement stmt = conn.prepareStatement(SUBMIT_FUEL_PURCHASE_REPORT);
			stmt.setDate(1, Date.valueOf(date));
			stmt.setInt(2,  gallons);
